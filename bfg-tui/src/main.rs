...
use tui::Terminal;
use thiserror::Error;// also see anyhow
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        terminal.draw(|rect| {
            ...
            let copyright = Paragraph::new("BFG-tui")
            ...
            Event::Input(event) => match event.code {
                ...
                KeyCode::Down => {
                    if let Some(selected) = pet_list_state.selected() {
                        let amount_pets = read_db().expect("can fetch pets db").len();
...
fn render_home<'a>() -> Paragraph<'a> {
    let home = Paragraph::new(vec![
        ...
        Spans::from(vec![Span::raw("")]),
        Spans::from(vec![Span::raw("Press 'p' to access pets, 'a' to add random new pts and 'd' to delete the currently selected")]),
    ])
...
fn render_pets<'a>(pet_list_state: &ListState) -> (List<'a>, Table<'a>) {
    let pets = Block::default()
        ...
        .title("Pets")
        .border_type(BorderType::Plain);
    let pet_list = read_db().expect("can not read db.json");
    let items: Vec<_> = pet_list
        .iter()
        .map(|pet| {
            ListItem::new(Spans::from(vec![Span::styled(
                pet.name.clone(), Style::default(),
            )]))
        }).collect();
