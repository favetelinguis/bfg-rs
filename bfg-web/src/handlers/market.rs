...
pub async fn get_market<T: BfgService>(service: web::Data<Mutex<T>>) -> HttpResponse {
    let market_details: MarketDetails = service.lock().unwrap().market_details().into();
...
pub async fn update_market<T: BfgService>(event: web::Json<MarketEvent>, service: web::Data<Mutex<T>>) -> HttpResponse {
    ...
    service.lock().unwrap().publish_update_event(Action::MarketEvent(market_event.into()));
