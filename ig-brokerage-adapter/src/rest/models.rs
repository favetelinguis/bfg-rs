...
/// I put on the magic word MYREF since autogenerated references if i place order in web platforms can start with a digit so
/// want to be sure to only process my references.
/// <digit to find which reference><MYREF to filter out only wo generated by bot><epic to make wo unique between markets>
...
impl CreateWorkingOrderRequest {
    pub fn new(direction: Direction, level: f64, reference: &str, market_info: MarketInfo, target_distance: f64, stop_distance: f64) -> Self {
        ...
        let deal_reference = format!("{}MYREF{}", ref_id, market_info.epic).replace(".", ""); // needed to have unique references
        Self {
...
impl Default for OpenPositionRequest {
    fn default() -> Self {
        Self {
            ...
            stop_distance: 0,
            limit_distance: 0,
            force_open: false, //netting if force open = false
