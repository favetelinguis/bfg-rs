...
/// The trade system also only care about events generated by the system, interactions in IG GUI should not generate events so we only
/// check channel PublicRestOTC
pub fn parse_trade_update(
    ...
    let conf = if parts[0].starts_with('{') {
        ...
        if let Some(mut confirmation) = serde_json::from_str::<TradeConfirmationUpdate>(parts[0]).ok() {
            if confirmation.deal_reference.contains("MYREF") {
                ...
                info!("Update is not for trading bot: {}", parts[0]);
                None }
        } else {
            warn!("Unable to deserialize CONFS: {}", parts[0]);
            None }
    } else {
        None
    };
    let opu = if parts[1].starts_with('{') {
        ...
        if let Some(mut confirmation) = serde_json::from_str::<OpenPositionUpdate>(parts[1]).ok() {
            if confirmation.deal_reference.contains("MYREF") {
                ...
                info!("Update not for trading bot: {}", parts[1]);
                None }
        } else { None }
...
/// Check if now is before conf time + duration
fn is_old_message(conf_time: String) -> bool {
    // Remove milliseconds
    let conf_time = &conf_time[..19];
    let conf_time = format!("{}Z", conf_time);
    let conf_time: DateTime<Utc> = DateTime::parse_from_rfc3339(conf_time.as_str()).unwrap().with_timezone(&Utc);
    let conf_time = conf_time.add(Duration::seconds(10));
    conf_time < Utc::now()
}
